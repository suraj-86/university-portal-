...
const AdminPayments = () => {
    ...
    if (loading) return <div className="p-10 text-center text-slate-400 dark:text-slate-500">Loading ledger data...</div>;

    return (
        <div className="p-6 md:p-10 bg-slate-50 dark:bg-slate-950 min-h-screen font-sans">
            <header className="mb-8">
                <h2 className="text-3xl font-bold text-slate-900 dark:text-slate-100">Payment Ledger</h2>
                <p className="text-slate-500 dark:text-slate-400 mt-1">Monitor transactional history and settlement statuses.</p>
            </header>
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
                <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between bg-slate-50/50 dark:bg-slate-900/50">
                    <input 
                        ...
                        className="pl-4 pr-4 py-2 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl text-sm outline-none"
                    />
                    <select value={methodFilter} onChange={(e) => setMethodFilter(e.target.value)} className="py-2 px-3 border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 text-slate-900 dark:text-slate-100 rounded-xl text-sm outline-none">
                        ...
                        <tbody className="text-sm divide-y divide-slate-100 dark:divide-slate-800 text-slate-800 dark:text-slate-200">
                            {filteredPayments.map((payment) => (
                                <tr key={payment.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                    <td className="p-4">
                                        <div className="font-bold text-slate-900 dark:text-slate-100">{payment.transaction_reference}</div>
                                        <div className="text-xs text-slate-500 dark:text-slate-400">{new Date(payment.payment_date).toLocaleString('en-IN')}</div>
                                    </td>
                                    <td className="p-4">
