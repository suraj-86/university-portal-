            ...
            <div className="space-y-4">
                {notices.length > 0 ? notices.map(notice => (
                    <div 
                        ...
                        className="bg-white hover:bg-slate-50 border border-slate-100 rounded-[24px] p-5 flex flex-col lg:flex-row lg:items-center gap-6 transition-all cursor-pointer group relative overflow-hidden shadow-sm"
                    >
                        <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${notice.priority === 'High' ? 'bg-rose-500' : 'bg-indigo-500'}`}></div>
                        
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2">
                                <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${notice.author_role === 'teacher' ? 'bg-indigo-50 text-indigo-600' : 'bg-slate-100 text-slate-500'}`}>
                            ...
                            <h3 className="text-lg font-black text-slate-800 group-hover:text-indigo-600 transition-colors mb-1 truncate capitalize">
                                {notice.title}
                            </h3>
                        ...
                        {notice.attachment_url && (
                            <div 
                                ...
                                className="shrink-0 flex items-center gap-4 bg-slate-50 p-3 rounded-2xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 transition-all group/file"
                            >
                                <div className="p-2 bg-white rounded-xl shadow-sm text-indigo-600 group-hover/file:bg-indigo-600 group-hover/file:text-white transition-colors">
                                    <FileText size={20} />
                                </div>
                                ...
                                <Download size={16} className="text-slate-300 group-hover/file:text-indigo-600" />
                            </div>
                        )}

                        <div className="flex items-center justify-between lg:justify-end gap-6 lg:border-l lg:border-slate-100 lg:pl-6 shrink-0">
                            <div className="p-2.5 bg-slate-50 text-slate-300 rounded-xl group-hover:bg-indigo-600 group-hover:text-white transition-all">
                                <ChevronRight size={20} />
                            </div>
                        </div>
                    </div>
                )) : <p className="text-slate-400 italic">No recent notices available for this ward.</p>}
            </div>
