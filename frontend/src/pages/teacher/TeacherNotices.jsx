...
const TeacherNotices = () => {
    ...
    const fetchData = async () => {
        try {
            const classRes = await fetch(`http://localhost:5000/api/teacher/${user.id}/assigned-subjects`);
            const classData = await classRes.json();
            setMyClasses(classData);

            const noticeRes = await fetch(`http://localhost:5000/api/teacher/${user.id}/notices`);
    ...
    const handleUpdateNotice = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch(`http://localhost:5000/api/notices/${editingNotice.id}`, {
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    ...
                    target_role: 'student',
                    subject_id: editingNotice.subject_id, // Updated to save the target class
                    priority: editingNotice.priority,
                    // If a new file is selected, use it. Otherwise keep the old attachment name.
                    attachment_url: editingNotice.new_attachment ? editingNotice.new_attachment.name : editingNotice.attachment_url
                })
            });
            if (response.ok) {
    ...
    const AttachmentCard = ({ fileName }) => (
        <div className="flex items-center justify-between bg-white w-full sm:w-80 px-4 py-3 rounded-xl border border-slate-200 mt-2 shadow-sm">
            ...
            <button onClick={() => handleDownloadFile(fileName)} className="p-2 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Download Document">
                <Download size={16} />
            </button>
        </div>
    );
    ...
    return (
        <div className="p-6 md:p-10 bg-slate-50 min-h-screen font-sans">
                            ...
                            <form onSubmit={handleSendNotice} className="flex flex-col gap-5">
                                <div>
                                    ...
                                    <select 
                                        ...
                                        className="w-full px-4 py-3 border border-slate-200 bg-slate-50 rounded-xl text-sm font-bold text-slate-700 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
                                    >
                                ...
                                <div>
                                    ...
                                    <textarea 
                                        ...
                                        className="w-full px-4 py-3 border border-slate-200 bg-slate-50 rounded-xl text-sm font-medium text-slate-700 focus:ring-2 focus:ring-indigo-200 outline-none transition-all resize-none" 
                                    />
                                </div>
                                <div>
                                    ...
                                    <div className="bg-white p-2 rounded-xl border border-dashed border-slate-300 flex flex-col items-center hover:bg-slate-50 transition-colors">
                                        <label className="flex flex-col items-center gap-2 cursor-pointer w-full py-4 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">
                                            <Paperclip size={20} className="text-indigo-400" /> 
                                            {attachment ? <span className="text-indigo-600">{attachment.name}</span> : 'Click to Upload PDF'}
            ...
            {isModalOpen && (
                <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Edit Announcement">
                    <form onSubmit={handleUpdateNotice} className="space-y-5">
                        {/* 1. Target Class Selection Added to Edit */}
                        <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Target Class</label>
                            <select 
                                required
                                value={editingNotice.subject_id || ''} 
                                onChange={(e) => setEditingNotice({...editingNotice, subject_id: e.target.value})} 
                                className="w-full px-4 py-3 border border-slate-200 bg-slate-50 rounded-xl text-sm font-bold text-slate-700 focus:ring-2 focus:ring-indigo-200 outline-none transition-all"
                            >
                                <option value="" disabled>-- Select a Class --</option>
                                {myClasses.map(cls => <option key={cls.id} value={cls.id}>{cls.course_name} - {cls.subject_name}</option>)}
                        ...
                        {/* 2. Attachment Upload Added to Edit */}
                        <div>
                            <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest block mb-1.5 ml-1">Attachment (Optional)</label>
                            <div className="bg-white p-2 rounded-xl border border-dashed border-slate-300 flex flex-col items-center hover:bg-slate-50 transition-colors">
                                <label className="flex flex-col items-center gap-2 cursor-pointer w-full py-3 text-[10px] font-bold text-slate-500 uppercase tracking-wider text-center">
                                    <Paperclip size={20} className="text-indigo-400" /> 
                                    {editingNotice.new_attachment ? (
                                        <span className="text-indigo-600">New File: {editingNotice.new_attachment.name}</span>
                                    ) : editingNotice.attachment_url ? (
                                        <span className="text-indigo-600">Current File: {editingNotice.attachment_url} (Click to change)</span>
                                    ) : (
                                        'Click to Upload PDF'
                                    )}
            ...
            {/* FIXED PREVIEW MODAL - PINNED HEADER & SCROLLABLE CONTENT */}
            {viewingNotice && (
                <Modal 
                ...
                >
                    <div className="flex flex-col gap-4 max-w-2xl mx-auto">
                        ...
                        {/* SCROLLABLE MIDDLE: Notice Content only */}
