...
const StudentNotices = () => {
    ...
    return (
        <div className="p-4 md:p-10 bg-[#fbfcfd] dark:bg-slate-950 min-h-screen font-sans">
            <header className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <div>
                    <h2 className="text-3xl font-black text-slate-900 dark:text-slate-100 tracking-tight flex items-center gap-2">
                        NOTICE <span className="text-blue-600 dark:text-blue-400 italic">UPDATES</span>
                    </h2>
                    <p className="text-slate-400 dark:text-slate-500 font-bold text-[10px] uppercase tracking-[0.2em] mt-1">Official University Announcements</p>
                </div>
                <div className="relative w-full md:w-80 group">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 dark:text-slate-600 group-focus-within:text-blue-600 transition-colors" size={18} />
                    <input 
                        type="text" placeholder="Search notices..." value={searchTerm}
                        onChange={(e) => setSearchTerm(e.target.value)}
                        className="w-full pl-11 pr-4 py-3 rounded-2xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 shadow-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all text-sm font-medium"
                    />
                </div>
            </header>
                ...
                {['All', 'Teacher', 'Admin'].map(type => (
                    <button 
                        ...
                        className={`px-6 py-2 rounded-xl text-[11px] font-black uppercase tracking-widest transition-all whitespace-nowrap ${
                            filterType === type ? 'bg-slate-900 dark:bg-slate-800 text-white shadow-lg' : 'bg-white dark:bg-slate-900 text-slate-400 dark:text-slate-500 border border-slate-100 dark:border-slate-800 hover:bg-slate-50 dark:hover:bg-slate-800'
                        }`}
                    >
            ...
            <div className="space-y-3">
                {loading ? (
                    <div className="py-20 text-center animate-pulse font-bold text-slate-300 dark:text-slate-600 uppercase text-[10px] tracking-widest">Loading Board...</div>
                ) : filteredNotices.length > 0 ? filteredNotices.map(notice => (
                    <div 
                        key={notice.id}
                        onClick={() => setViewingNotice(notice)}
                        className="bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800/80 border border-slate-100 dark:border-slate-800 rounded-[24px] p-5 flex flex-col lg:flex-row lg:items-center gap-6 transition-all cursor-pointer group relative overflow-hidden"
                    >
                        <div className={`absolute left-0 top-0 bottom-0 w-1.5 ${notice.priority === 'High' ? 'bg-rose-500' : 'bg-blue-500'}`}></div>
                        
                        <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-3 mb-2">
                                <span className={`text-[9px] font-black uppercase px-2 py-0.5 rounded-md ${notice.author_role === 'teacher' ? 'bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'}`}>
                            ...
                            <h3 className="text-lg font-black text-slate-800 dark:text-slate-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mb-1 truncate capitalize">
                                {notice.title}
                            </h3>
                        ...
                        {notice.attachment_url && (
                            <div 
                                ...
                                className="shrink-0 flex items-center gap-4 bg-slate-50 dark:bg-slate-800 p-3 rounded-2xl border border-slate-100 dark:border-slate-700 hover:border-blue-200 dark:hover:border-blue-800 hover:bg-blue-50 dark:hover:bg-blue-950/50 transition-all group/file"
                            >
                                <div className="p-2 bg-white dark:bg-slate-900 rounded-xl shadow-sm text-blue-600 dark:text-blue-400 group-hover/file:bg-blue-600 group-hover/file:text-white transition-colors">
                                    <FileText size={20} />
                                </div>
                                ...
                                <Download size={16} className="text-slate-300 dark:text-slate-600 group-hover/file:text-blue-600" />
                            </div>
                        )}
                        ...
                        <div className="flex items-center justify-between lg:justify-end gap-6 lg:border-l lg:border-slate-100 dark:lg:border-slate-800 lg:pl-6 shrink-0">
                            {notice.author_role !== 'admin' && (
                                <div className="flex items-center gap-2">
                                    <div className="w-8 h-8 rounded-full bg-slate-900 dark:bg-slate-800 flex items-center justify-center text-[10px] font-black text-white uppercase">
                                        {notice.author_name?.charAt(0)}
                                    </div>
                                    <p className="text-[11px] font-black text-slate-500 dark:text-slate-400 uppercase tracking-tight">{notice.author_name}</p>
                                </div>
                            )}
                            <div className="p-2.5 bg-slate-50 dark:bg-slate-800 text-slate-300 dark:text-slate-600 rounded-xl group-hover:bg-blue-600 group-hover:text-white transition-all">
                    ...
                    <div className="py-32 text-center bg-white dark:bg-slate-900 rounded-[32px] border-2 border-dashed border-slate-100 dark:border-slate-800">
                        <Megaphone size={40} className="mx-auto text-slate-100 dark:text-slate-800 mb-4" />
                        <p className="text-slate-300 dark:text-slate-600 font-bold uppercase tracking-widest text-[10px]">No notices found</p>
                    </div>
                )}
            </div>
                    ...
                    <div className="flex flex-col h-full max-h-[80vh]">
                        <div className="shrink-0 pb-4 border-b border-slate-100 dark:border-slate-800">
                            <div className="flex items-center justify-between mb-2">
                                <span className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${viewingNotice.priority === 'High' ? 'bg-rose-50 dark:bg-rose-950/50 text-rose-600 dark:text-rose-400' : 'bg-blue-50 dark:bg-blue-950/50 text-blue-600 dark:text-blue-400'}`}>
